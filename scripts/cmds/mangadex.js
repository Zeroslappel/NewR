      ...
      if (type == "search") {
        ...
        if (Reply.pagetype == true) {
          if (args[0]?.toLowerCase() === "page" && args[1] > 0) {
            ...
            return args[0]?.toLowerCase() == "done" ? api.unsendMessage(messageID) && api.setMessageReaction('✅', event.messageID, () => {}, true) : api.sendMessage("Invalid input!⚠\nEx: Page 2/Select 2/Done", event.threadID, event.messageID);
          }
        }
        ...
        if (type == 'read') {
          ...
          api.setMessageReaction('⏳', event.messageID, async () => {
            try {
              ...
              return api.sendMessage("Something went wrong", event.threadID, event.messageID) && api.setMessageReaction('⚠', event.messageID, () => {}, true);
            }
          }, true);
        }
      }
    } catch (error) {
      return api.sendMessage("Error: " + error, event.threadID, event.messageID) && api.setMessageReaction('⚠', event.messageID, () => {}, true);
    }
  }
};
